...
Now the important variables seems more reasonable. Let's take a quick look at the first one with 100% accuracy, min_roll_forearm.

```{r}
summary(training$min_roll_forearm)
```

What the hell? Most of them are NA's? The same with the second variable: var_total_accel_belt. Then I realize there are some summary variables for each window (new_window=='yes'). Since number of windows are too few (406) and information was lost during those summarizing calculation, here I only include the original descriptive variables in my analysis.
